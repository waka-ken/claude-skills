...
// ─────────────────────────────────────────────
function processMinutes_(notionToken, geminiApiKey, _today) {
...
// ─────────────────────────────────────────────
function createTask_(
  token,
  { simpleName, originalText, projectRelation, dueDate, minutesTitle, minutesDate }
) {
  const properties = {
...
function setupDailyTrigger() {
  ScriptApp.getProjectTriggers()
    .filter(t =>
      ['runDailyTaskReport', 'runExtractor', 'runDailyAiOrganizer'].includes(t.getHandlerFunction())
    )
